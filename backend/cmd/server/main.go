...
func main() {
	...
	adminPath, adminSvc := golden_fishv1connect.NewAdminServiceHandler(adminHandler, compress1KB)

	mux.Handle(productPath, productSvc)
	mux.Handle(userPath, userSvc)
	mux.Handle(cartPath, cartSvc)
	mux.Handle(orderPath, orderSvc)
	mux.Handle(adminPath, adminSvc)
	...
	checker := grpchealth.NewStaticChecker(
		...
		golden_fishv1connect.AdminServiceName,
	)
