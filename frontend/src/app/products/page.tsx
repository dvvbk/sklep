...
import type { SearchProductsResponse, ListProductsResponse } from "@/gen/golden_fish/v1/products_pb";
...
function ProductsContent() {
  ...
  // Gdy kategorie się załadują, zamień slug z URL na UUID
  useEffect(() => {
    if (!categoriesData || !categorySlug) return;
    const match = categoriesData.categories.find((c) => c.slug === categorySlug);
    setCategoryId(match?.id ?? "");
    setPage(1);
  }, [categoriesData, categorySlug]);

  const { data, isLoading } = useQuery<SearchProductsResponse | ListProductsResponse>({
